...
class AgentTripwire(gl.contract.Contract):
 ...
 def _fetch(self,url):
  ...
  if len(raw)>14000: raise gl.vm.UserError('[EXPECTED] source exceeds 14000-byte supported limit')
 ...
 def submit_recovery(self,tripwire_id:str,recovery_url:str)->None:
  ...
  def run():
   ...
   return {'resolved':resolved,'resolved_indexes':indexes,'policy_digest':policy_digest,'action_digest':action_digest,'digest':digest}
  def validate(leader):
   if not isinstance(leader,gl.vm.Return): return False
   try: return run()==leader.calldata
   except: return False
